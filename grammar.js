...
module.exports = grammar(require("tree-sitter-json/grammar"), {
	...
	rules: {
		...
		_rule_value_array: ($) =>
			squareBracketScoped(
				choice($.null, commaSep1($.number), commaSep1($.string)),
			),
		rule_value_matching: ($) =>
			seq(
				alias($.string, $.rule_constant_value),
				":",
		...
		rule_or_matching: ($) =>
			seq(
				...
				seq(
					":",
					choice(
						squareBracketScoped(commaSep1($.object)),
						curlyBracketScoped(commaSep2($.rule_value_matching)),
					),
				),
			),
